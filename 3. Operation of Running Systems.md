...
- `sudo systemctl reboot/poweroff` - to reboot/shutdown as non-root user
- `sudo systemctl reboot/poweroff --force` - to force reboot/shutdown as non-root user
- `sudo systemctl poweroff --force --force` - to double force  reboot/shutdown as non-root user
...
- `sudo systemctl set-default multi-user.target` to change default boot target
  - `graphical.target` - allows for multi users, disables graphical user interface
  ...
  - `rescue.target` - target somewhere b/w emergency and multi-user
...
init - initialization system for starting following units:
...
- `35 6 * * *` - when cron job should be run
...
- `which` to identify full path of app
- `crontab -e` to modify user crontab
- `35 6 * * * /usr/bin/touch test_passed`
- `crontab -l` to see the crontab for the current user
- `sudo crontab -l` to see the crontab for the root user
- `sudo crontab -e -u [USER]` to edit crontab for other user
- `sudo crontab -r` to remove crontab
- `sudo crontab -r -u [USER]` to remove crontab for specific user
...
- `sudo dnf provides docker` - provides info which package provides the command
